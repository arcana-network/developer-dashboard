...
<template>
  <div class="relative cursor-pointer notification" @click="emits('click')">
    <div
      v-if="areUnreadNotificationAvaiable"
      class="absolute top-px-8 right-px-8 flex items-center justify-center min-w-px18 min-h-px18 p-px1 text-px10 rounded-full text-white_solid-default bg-cornflower_blue-default hover:bg-fairy_dust-default"
    >
      <span class="z-overlay">1</span>
      <span
        class="absolute w-px16 h-px16 bg-white_solid-default rounded-full animate-ripple bg-cornflower_blue-default"
      ></span>
    </div>
    <svg
      width="24"
      ...
      class="cursor-pointer w-px22 hover:text-fairy_dust-default"
