<template>
  <section style="margin-top: 3em" v-if="isConfigured">
    <v-stack direction="column" gap="1em" align="start">
      <h2>APP NAME</h2>
      <v-text-field v-model="appName" strong class="app-name" :messageType="appNameError ? 'error' : ''"
        message="App name is required" />
    </v-stack>
  </section>
  <v-card class="app-name-card" v-else :id="'configure-step-' + 1">
    <v-stack direction="row" sm-direction="column" gap="1.2em" sm-gap="1.2em" style="flex-grow: 1">
      <h4 style="margin-top: 1.5em; width: 25vw; min-width: 280px">
        ENTER APP NAME
      </h4>
      <v-text-field v-model="appName" strong style="width: 20vw; min-width: 300px; max-width: 400px; margin-top: 1em"
        :messageType="appNameError ? 'error' : ''" message="App name is required" @keyup.enter="onEnterClick"
